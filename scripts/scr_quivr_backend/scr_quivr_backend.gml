...
function __quivr_network_async(){
	...
	switch(_type){
		...
		case network_type_data:
			...
			if (string_pos("POST", _string) != 0) { // Packet contains OAuth token.
				...
				__quivr_log("Auth Token Recieved! ( " + string(q.auth_key) + " )");
...
function __quivr_validate_config(){
	// TODO
	return true;
}
...
function __quivr_generate_config(){
	return {
		store_auth_key_locally : true
	}
}